...
public final class InputState implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    private final boolean[] pressedMouse = new boolean[8];
    ...
    public synchronized boolean consumeKey(int code) {
        if (code < 0 || code >= pressedKeys.length) return false;
        boolean value = pressedKeys[code];
        pressedKeys[code] = false;
        return value;
    }

    public synchronized boolean mouseDown(int button) {
        return button >= 0 && button < mouse.length && mouse[button];
    }

    public synchronized boolean consumeMouse(int button) {
        if (button < 0 || button >= pressedMouse.length) return false;
        boolean value = pressedMouse[button];
    ...
    public synchronized double[] consumeMouseDelta() {
        double[] value = {mouseDx, mouseDy};
        mouseDx = mouseDy = 0;
        return value;
    }
    ...
    public void dispatchKeyPresses(java.util.function.IntConsumer handler) {
    ...
    public synchronized void releaseAll() {
        ...
        Arrays.fill(pressedMouse, false);
        mouseDx = mouseDy = 0;
        wheel = 0;
    }

    public void setCaptured(boolean value) {
        if (canvas == null || captured == value) return;
        captured = value;
        lastX = lastY = Integer.MIN_VALUE;
        if (value) {
            BufferedImage image = new BufferedImage(16, 16, BufferedImage.TYPE_INT_ARGB);
            Cursor invisible = Toolkit.getDefaultToolkit().createCustomCursor(image, new Point(0, 0), "echo-hidden");
            canvas.setCursor(invisible);
            centerMouse();
            canvas.requestFocusInWindow();
        } else {
            canvas.setCursor(normalCursor == null ? Cursor.getDefaultCursor() : normalCursor);
        }
    }
    ...
    private void centerMouse() {
        if (robot == null || canvas == null || !canvas.isShowing()) return;
        try {
            Point screen = canvas.getLocationOnScreen();
            int x = screen.x + canvas.getWidth() / 2;
            int y = screen.y + canvas.getHeight() / 2;
            warping = true;
            robot.mouseMove(x, y);
    ...
    public synchronized void keyPressed(KeyEvent e) {
        ...
        if (code >= 0 && code < keys.length) {
            if (!keys[code]) pressedKeys[code] = true;
            keys[code] = true;
        }
    }
    ...
    public synchronized void mousePressed(MouseEvent e) {
        ...
        if (b >= 0 && b < mouse.length) {
            if (!mouse[b]) pressedMouse[b] = true;
            mouse[b] = true;
        }
    ...
    private void motion(MouseEvent e) {
        synchronized (this) {
            mouseX = e.getX();
            mouseY = e.getY();
            if (captured && robot != null && canvas != null && canvas.isShowing()) {
                try {
                    Point screen = canvas.getLocationOnScreen();
                    int cx = screen.x + canvas.getWidth() / 2;
                    int cy = screen.y + canvas.getHeight() / 2;
                    int dx = e.getXOnScreen() - cx;
                    int dy = e.getYOnScreen() - cy;
                    if (warping && Math.abs(dx) <= 1 && Math.abs(dy) <= 1) {
                        warping = false;
                        return;
                    }
                    mouseDx += dx;
